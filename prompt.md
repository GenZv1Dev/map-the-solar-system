...
why did you limit the max count to 200000 I added a zero now it finally loaded them all but its lagging really bad we need to optimize it more, and the lava isn't showing on the surface of the sun, and the camera move speed is still wayyy too fast, and the time scale is still not realistic and too fast, and it still shows NEO 0 and PHA 0 in hte Aasteroids tab, and I still dont see the bvh controls to useBVH and useLOD and freeze and the prices in the asteroids tab are still too round it makes no sense they are all at $1000.0Q this number is also not readable make something us humans understand...
also search mode needs a progress bar while searching, and the speed shouldn't be moving in the indicator ui, why does it show it in c unity idk what is c just show me the current speed like 0.001, and the ui that shows that speed and time scale and sim elapsed and sim date is on top of the astreoid info panel it needs to move the astreoid info panel to the left side, also the astreoids labels when you go close to them has disappears bring them back, and I still fly too fast make me fly a hundred times slower by default, and the sim date and sim elasped doesn't even work